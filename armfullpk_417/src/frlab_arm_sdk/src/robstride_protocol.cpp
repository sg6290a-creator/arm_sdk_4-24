...
{
    return parseFeedback(arb_id, data,
                         RS_TORQUE_MIN, RS_TORQUE_MAX,
                         RS_VEL_MIN,    RS_VEL_MAX);
}
                                                ...
                                                float vel_min,    float vel_max)
{
    ...
    state.velocity_rads = uint16ToFloat(vel_raw,   vel_min,   vel_max);
    state.torque_nm     = uint16ToFloat(torq_raw,  torque_min, torque_max);
